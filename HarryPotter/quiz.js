  ...
  for (let i = 0; i < optionBoxes.length; i++) {
    ...
    if (x >= box.x && x <= box.x + box.width && y >= box.y && y <= box.y + box.height) {
      ...
      optionBoxes.forEach((box, index) => {
        ctx.fillStyle = index === i ? (isCorrect ? "green" : "red") : "#f0f0f0";
        ctx.fillRect(box.x, box.y, box.width, box.height);
        ctx.fillStyle = "black";
        wrapText(questions[currentQuestion].options[index], box.x + 20, box.y + 32, box.width - 40, 30);
      });
...
function showResult() {
  ...
  ctx.fillStyle = "black";
  ctx.font = "30px Arial";
  wrapText("Quiz beendet!", 50, 100, canvas.width - 100, 40);
  wrapText(`Du hast ${score} von ${questions.length} Punkten erreicht.`, 50, 160, canvas.width - 100, 40);
  ...
  wrapText(resultText, 50, 220, canvas.width - 100, 40);
